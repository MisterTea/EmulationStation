...
DetailedGameListView::DetailedGameListView(Window* window, const FileData& root, SystemData* system) :
    ...
    if (metadata.get("image").length()) {
      ...
      mImage.setImage("");
		...
		if(file->getType() == GAME)
		{
			...
			mFavorite.setValue(metadata.get("favorite"));
		}
...
std::vector<HelpPrompt> DetailedGameListView::getHelpPrompts()
{
	...
	prompts.push_back(HelpPrompt(inputCategoryToString(INPUT_4B_UP), "favorite"));
