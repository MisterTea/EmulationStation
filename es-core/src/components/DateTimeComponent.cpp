...
bool DateTimeComponent::input(InputConfig* config, Input input)
{
	...
	if(mEditing)
	{
		if(config->isMappedTo(INPUT_4B_DOWN, input))
		{
			mEditing = false;
		...
		int incDir = 0;
		if(config->isMappedTo(INPUT_UP, input) || config->isMappedTo(INPUT_L1, input))
			incDir = 1;
		else if(config->isMappedTo(INPUT_DOWN, input) || config->isMappedTo(INPUT_R1, input))
			incDir = -1;
